...
from Shared import constants, logic_procedural, render__procedural
import logic_procedural
import render__procedural
...
__lines = 0
__figures = 0
__desk = logic_procedural.create_desk(constants.BLOCKS_V, constants.BLOCKS_H)
__desk_next = logic_procedural.create_desk(constants.NEXT_V, constants.NEXT_H)
__gamePaused = False
__gameOver = False
__figure = copy.deepcopy(constants.FIGURES[random.randint(0, constants.FIGURES_COUNT-1)])
__figure = logic_procedural.set_figure_colour(__figure.copy())
__figures += 1
...
__figure_next = logic_procedural.set_figure_colour(__figure_next.copy())
...
while 1:
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN and __gameOver:
            ...
            __lines = 0
            __figures = 0
            __desk = logic_procedural.create_desk(
                constants.BLOCKS_V, constants.BLOCKS_H)
            __gameOver = False
            __figure = copy.deepcopy(
        ...
        if event.type == pygame.KEYDOWN  and not __gameOver:
            ...
            if event.key == pygame.K_DOWN and mods ==64:
                ...
                __desk = logic_procedural.add_empty_line(copy.deepcopy(__desk))
            ...
            if (event.key == pygame.K_RIGHT or event.key == pygame.K_KP6) and mods == 0:
                if not logic_procedural.check_intersect(__desk, __figure, [__fig_pos[0], __fig_pos[1] + 1]):
                    __fig_pos[1] += 1
            ...
            if (event.key == pygame.K_5 or event.key == pygame.K_UP or event.key == pygame.K_KP5) and mods == 0:
                tmp_rotated, tmp_pos = logic_procedural.rotate(__figure, __fig_pos)
                ...
                if not logic_procedural.check_intersect(__desk, tmp_rotated, tmp_pos):  # simple turn on an empty space
            ...
            if (event.key == pygame.K_SPACE or event.key == pygame.K_DOWN or event.key == pygame.K_KP2) and mods == 0:
                if not logic_procedural.check_intersect(__desk, __figure, [__fig_pos[0] + 1, __fig_pos[1]]):
                    __fig_pos[0] += 1
                else:
    ...
    render__procedural.render_desk(screen, __desk, 0, 0)
    render__procedural.render_desk(screen, __desk_next,
                                 (constants.BLOCKS_H + constants.NEXT_OFFSET) * (
                                 constants.BLOCKSIZE + constants.SPACER),
                                 (constants.BLOCKSIZE + constants.SPACER))

    render__procedural.render_figure(screen, __desk, __figure, __fig_pos, 0, 0)

    render__procedural.render_figure(screen, __desk, __figure_next,
    ...
    if __gamePaused:
        render__procedural.render_pause(screen)

    if __gameOver:
        render__procedural.render_game_over(screen, __score)
        ...
        if tick >= constants.maxTicks:
            if 1: #not logic_structure.check_intersect(__desk, __figure, [__fig_pos[0], __fig_pos[1]]):
                if not logic_procedural.check_intersect(__desk, __figure, [__fig_pos[0] + 1, __fig_pos[1]]):
                    ...
                    __desk = logic_procedural.merge(__desk, __figure, __fig_pos)
                    __desk, lines = logic_procedural.check_line(copy.deepcopy(__desk))
                    if lines > 0:
                        __lines += lines
                    ...
                    __figures += 1
                    __fig_pos = [0, int(constants.BLOCKS_H / 2 - len(__figure[0]) / 2)]
                    __figure_next = copy.deepcopy(
                        constants.FIGURES[random.randint(0, constants.FIGURES_COUNT - 1)])
                    __figure_next = logic_procedural.set_figure_colour(__figure_next.copy())
                    if logic_procedural.check_intersect(__desk, __figure, [__fig_pos[0], __fig_pos[1]]):
                        __gameOver = True
                        pygame.key.set_repeat(0, 0)

            tick = 0

        render__procedural.render_info(screen,
                                     [str(constants.BLOCKS_V)+" x " + str(
                                         constants.BLOCKS_H),__score,__figures,__level,__lines,__destroyed],
